        ...
        <div class="col-sm-8 text-left container">
            <div class="table-wrapper">
                ...
                <div class="table-title row">
                    <h2 class="title"><spring:message code="shop.prescriptions.title"/></h2>
                ...
                <table class="table table-striped table-hover">
                    <thead>
                    <tr>
                        <th class="col-md-8 text-column"><spring:message code="shop.prescriptions.id"/></th>
                        <th class="col-md-8 text-column"><spring:message code="shop.prescriptions.name"/></th>
                        <th class="col-md-2 number-column"><spring:message code="shop.prescriptions.diagnosis"/></th>
                        <th class="col-md-2 number-column"><spring:message code="shop.prescriptions.date"/></th>
                        <th class="col-md-2 number-column"></th>
                        <th class="col-md-2 number-column"></th>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${page.content}" var="prescription">
                        <tr>
                            <th>${prescription.prescriptionId}</th>
                            <th>${prescription.name}</th>
                            <th>${prescription.diagnosisName}</th>
                            <th>${prescription.prescriptionDate.format(foramter)}</th>
                            <th>
                                <form method="POST" id="buyForm" action="">
                                    <input name="${_csrf.parameterName}" value="${_csrf.token}" type="hidden">
                                    <input name="prescriptionId" type="hidden" value = "${prescription.prescriptionId}"/>
                                    <input type="submit" class="btn btn-primary" value="<spring:message code="shop.prescriptions.delete"/>"/>
                                </form>
                            </th>
                            <th><a class="btn btn-primary" href="/shop/0?nameSearch=${fn:trim(prescription.name)}" role="button">
                                <spring:message code="shop.prescriptions.search"/></a>
                            </th>
                        </tr>
                    </c:forEach>
                    </tbody>
